...
test("드라이런 사이클: 매수신호가 나도 실주문은 전송되지 않는다", async () => {
  ...
  // 강한 하락 추세 → RSI 0 → BUY 신호 유도
  const descendingCloses = Array.from({ length: 20 }, (_, i) => ({ close: 200 - i, timestamp: i }));
  ...
  globalThis.fetch = (async (input: string | URL | Request, init?: RequestInit) => {
    ...
    if (url.includes("/api/v1/market-calendar/KR")) return json({ isOpen: true });
